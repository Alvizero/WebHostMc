...
type VersioneServerDisponibile = {
  ...
  nome: string;
  tipo: string;
  ultima_versione: number; // 0 o 1
  popolare: number;        // 0 o 1
};
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-center mb-8">
                    <div className="bg-slate-800 rounded-full p-1 flex items-center border border-gray-600 relative overflow-hidden">
                      ...
                      <div 
                        className={`absolute top-1 bottom-1 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full transition-all duration-500 ease-in-out shadow-lg ${
                          isNewUser ? 'left-1 w-[calc(50%-2px)]' : 'left-1/2 w-[calc(50%-2px)]'
                        }`}
                      />
                      
                      <button
                        onClick={() => setIsNewUser(true)}
                        className={`relative z-10 px-4 py-2 rounded-full font-medium text-sm transition-all duration-300 transform hover:scale-105 ${
                          isNewUser 
                            ? 'text-white' 
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className={`space-y-2 transition-all duration-500 ease-in-out delay-100 ${
                      isNewUser 
                        ? 'opacity-100 translate-y-0 max-h-96' 
                        : 'opacity-0 translate-y-4 max-h-0 overflow-hidden'
                    }`}>
                    ...
                    <div className={`space-y-2 transition-all duration-500 ease-in-out ${
                      isNewUser ? 'delay-300' : 'delay-100'
                    }`}>
                      <label className="block text-sm font-medium text-gray-300">Password</label>
                      <div className="relative">
                        <input
                          type={showPassword ? "text" : "password"}
                          className={`w-full bg-slate-800 border rounded-xl p-4 pr-12 text-white placeholder-gray-400 focus:ring-2 focus:ring-purple-500/20 transition-all duration-200 ${
                  ...
                  {selectedEgg && (
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          {filteredVersions
                            .filter(version => version.tipo === selectedEgg)
                            .map((version) => (
                              <button
                                key={version.nome}
                                onClick={() => setServerConfig({ ...serverConfig, version: version.nome })}
                                className={`p-3 rounded-xl border-2 transition-all duration-200 text-left hover:scale-105 ${
                                  serverConfig.version === version.nome
                                    ? "border-purple-500 bg-gradient-to-br from-purple-500/20 to-pink-500/20 text-purple-400 shadow-lg shadow-purple-500/25"
                                    : "border-gray-600 bg-slate-800 text-gray-300 hover:border-gray-500 hover:bg-slate-700"
                                }`}
                              >
                                <div className="font-bold text-sm">{version.nome.split(" ").pop()}</div>
